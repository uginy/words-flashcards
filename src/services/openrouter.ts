...
const refinementSystemPrompt = `You are an expert Hebrew linguist. Your task is to analyze and improve the provided Hebrew word data.
...
Верни улучшенную версию в том же JSON формате:
{
  ...
  try {
    ...
    if (!completion?.choices || !completion?.choices?.length || !completion?.choices[0]?.message?.content) {
